...
def main(filename, avg):
    ...
    tvec = get_translation(stack, avg)
    corrected = correct_motion(stack, tvec)
    ...
    tifffile.imsave(fn, corrected[1:].mean(axis = 0).astype('uint16'))
...
def get_translation(stack, avg):
    #avg = stack[1:].mean(axis = 0)
    f1 = np.fft.fft2(avg)
